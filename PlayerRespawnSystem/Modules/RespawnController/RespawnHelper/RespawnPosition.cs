...
namespace PlayerRespawnSystem
{
    ...
    {
        public static Vector3 GetSpawnPositionAroundTeleporter(
        ...
        )
        {
            ...
            return new Vector3(
                positionAroundTP.x,
                positionAroundTP.y + RoR2.Util.GetBodyPrefabFootOffset(body.gameObject),
                positionAroundTP.z
            );
        }

        public static Vector3 GetSpawnPositionAroundMoonBoss(
            RoR2.CharacterBody body,
            float minSpawnRadius,
            float maxSpawnRadius
        )
        ...
        {
            var gauntletIndexField = typeof(RoR2.VoidRaidGauntletController).GetField(
                "gauntletIndex",
                BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.GetField
            );
            var initialDonutField = typeof(RoR2.VoidRaidGauntletController).GetField(
                "initialDonut",
                BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.GetField
            );
            var followingDonutsField = typeof(RoR2.VoidRaidGauntletController).GetField(
                "followingDonuts",
                BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.GetField
            );

            if (
                gauntletIndexField == null
                || initialDonutField == null
                || followingDonutsField == null
            )
            {
                PlayerRespawnSystemPlugin.Log.LogWarning(
                    "Failed to find VoidRaidGauntletController field"
                );
                return Vector3.zero;
            }
            ...
            var currentDonutIdx = (int)
                gauntletIndexField.GetValue(RoR2.VoidRaidGauntletController.instance);
            if (currentDonutIdx <= 0)
            {
                currentDonut =
                    initialDonutField.GetValue(RoR2.VoidRaidGauntletController.instance)
                    as RoR2.VoidRaidGauntletController.DonutInfo;
                if (currentDonut == null)
                {
                    PlayerRespawnSystemPlugin.Log.LogWarning(
                        "Failed to get VoidRaidGauntletController::initialDonut field"
                    );
                    return Vector3.zero;
                }
            }
            ...
            {
                var followingDonuts =
                    followingDonutsField.GetValue(RoR2.VoidRaidGauntletController.instance)
                    as RoR2.VoidRaidGauntletController.DonutInfo[];
                if (followingDonuts == null)
                {
                    PlayerRespawnSystemPlugin.Log.LogWarning(
                        "Failed to get VoidRaidGauntletController::followingDonuts field"
                    );
                    return Vector3.zero;
                }
        ...
        private static bool TryUpdateToProperPositionOnStage(
            ref Vector3 position,
            float maxRadiusAroundPosition
        )
        {
            ...
            if (
                Physics.SphereCast(
                    ray,
                    maxRadiusAroundPosition,
                    out raycastHit,
                    maxDistance,
                    RoR2.LayerIndex.world.mask
                )
            )
            {
